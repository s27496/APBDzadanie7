...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            int result = arr.GroupBy(i => i).First(g => g.Count() % 2 == 1).Key;
            return result;
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            IEnumerable<Dept> result =
                Depts.Where(d =>
                    Emps.GroupBy(e => e.Deptno).Where(g => g.Count() == 5 || g.Count() == 0).Select(g => g.Key)
                        .Contains(d.Deptno)).OrderBy(d => d.Dname);
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Where(e => e.Job.Contains("A")).GroupBy(e => e.Job)
                .Where(g => g.Count() > 2)
                .OrderByDescending(g => g.Count()).Select(g => new { g.Key, LiczbaPracownikow = g.Count() });
            return result;
        }
        ...
        {
            // za https://stackoverflow.com/questions/4073713/is-there-a-good-linq-way-to-do-a-cartesian-product
            IEnumerable<object> result = new object[] { from e in Emps from d in Depts select new { e, d } }.Distinct();
            return result;
        }
    }
}
